...
public class GroupAdapter extends RecyclerView.Adapter {
    ...
    private boolean AUTH_EDIT_BASIC=false;
    private boolean AUTH_EDIT_GROUP=false;
    private boolean AUTH_EDIT_MACHINE=false;
    private boolean AUTH_EDIT_DISCOUNT=false;
    private boolean AUTH_EDIT_ADS=false;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof GroupBasicInfoViewHolder) {
            ...
            groupSettingViewHolder.setData(context, groupInfo,AUTH_EDIT_BASIC,AUTH_EDIT_DISCOUNT,mOnInfoClickListener);
        } else if (holder instanceof GroupPicViewHolder) {
            GroupPicViewHolder picViewHolder = (GroupPicViewHolder) holder;
            picViewHolder.setData(context, groupInfo.getAdPics(),AUTH_EDIT_ADS,mOnInfoClickListener);
        }
    }
   ...
   public void setOnEditInfoClickListener(OnInfoClickListener onInfoClickListener){
    ...
    private void resolveAuth() {
        ...
        List<String> auth = groupInfo.getAuth();
        if(auth.contains("1.0")){
            AUTH_EDIT_BASIC=true;
        }
        if(auth.contains("1.1")){
            AUTH_EDIT_GROUP=true;
        }
        if(auth.contains("1.2")){
            AUTH_EDIT_MACHINE=true;
        }
        if(auth.contains("1.3")){
            AUTH_EDIT_DISCOUNT=true;
        }
        if(auth.contains("1.4")){
            AUTH_EDIT_ADS=true;
        }
    }
