...
public class MenuPackViewHolder extends RecyclerView.ViewHolder {
    ...
    @BindView(R.id.tv_check_more_pack)
    TextView mTvCheckMorePack;
    @BindView(R.id.tv_add_pack)
    TextView mTvAddPack;
    @BindView(R.id.ll_more_packs)
    LinearLayout mLlMorePacks;
    ...
    public void setMenuPackData(
        ...
        View foot = mLlPackPackContainer.getChildAt(mLlPackPackContainer.getChildCount() - 1);
        ...
        if (packs == null || packs.size() == 0) {
            if(!AUTH_DELET_ADD_MENU_ITEM){
                ...
                mLlPackPackContainer.setLayoutParams(layoutParams);
                mLlPackPackContainer.setVisibility(View.GONE);
            }else {
